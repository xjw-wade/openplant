...
{
    int id;				// 点ID号
    int t_firstAlarm;	// 首次报警时间
    int t_levelChanged;	// 变到当前级别的时间
...
typedef struct {
	int id;				//点序号[ID]
	int t_firstAlarm;	//首次报警时间[TF]
	int t_levelChanged;	//变到当前报警级别的时间[TA]
	int time;			//点时间[TM]（实时报警），报警时间（历史报警）
    short status;       //点状态[DS]
    double value;       //点数值[AV]
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_value_byid(OpenPlant op, int num, const int *id, int* time, short *status, double *value, int *errors);
...
GMacroAPI(int) op2_get_archive_byname(OpenPlant op, OPGroup gh, int *value_type, int *begin_tm, int *end_tm, int *interval, OPResult *result, int *errors);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_history_byid(OpenPlant op, int num, const int *id, const int *value_type, int begin_tm, int end_tm, int interval, OPResult *result, int *errors);

GMacroAPI(int) op2_get_archive_byid(OpenPlant op, int num, const int *id, const int *value_type, int *begin_tm, int *end_tm, int *interval, OPResult *result, int *errors);
...
GMacroAPI(int) op2_write_archive(OpenPlant op, int pt, int num, const int *id, const int* time, const short *status, const double *value, int *errors);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_alarm_byid(OpenPlant op, int num, int *id, int begin_tm, int end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_alarm_byname(OpenPlant op, OPGroup gh, int begin_tm, int end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_child_alarm_byid(OpenPlant op, int id, int begin_tm, int end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_child_alarm_byname(OpenPlant op, const char *obj_name, int begin_tm, int end_tm, OPResult *result);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_update_alarm(OpenPlant op, int pt, int num, const int *id, const int *time, const short *status, const double *value, int *errors);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_value(OpenPlant op, int num, const char *name[], int *time, short *status, double *value, int *errors);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_snapshot(OpenPlant op, int num, const char *name[], int time, short *status, double *value, int *errors);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_archive(OpenPlant op, int num, const char *name[], const int *beginTime, const int *endTime, const int *interval, tsvResult *pResult[], int *error);
...
*         错误 返回 其他
*/
GMacroAPI(int) op2_get_archive_plot(OpenPlant op, int num, const char *name[], const int *beginTime, const int *endTime, const int *interval, tsvResult *pResult[], int *error);
...
GMacroAPI(int) op2_get_hist_alarm(OpenPlant op, int beginTime, int endTime, const char *tagName, int level, AlarmResult **pResult);


// 取历史报警：按条件过滤取记录数，便于分页
GMacroAPI(int) op2_get_hist_alarm_count(OpenPlant op, int beginTime, int endTime, const char *filter, size_t *count);


// 取历史报警：按条件过滤，按limit, offset取分页
GMacroAPI(int) op2_get_hist_alarm_filter(OpenPlant op, int beginTime, int endTime, const char *filter, size_t limit, size_t offset, AlarmResult **pResult);
